...
**The lottery will not run.** If Run Preview errors out or hangs, if the results look inconsistent with what the Preflight readiness panel predicted, or if you have any doubt about which policy version actually governed the run, stop. Do not finalize. Do not re-run repeatedly hoping it clears on its own, repeated re-runs can mask a real problem and make it harder to reconstruct what actually happened. Write down exactly what you saw: what you clicked, what error or inconsistency appeared, and when. Then escalate to your system administrator rather than working around it. If you need to try the mechanics again while this gets sorted out, use a Test rehearsal run, not a real one; a rehearsal can never be finalized as official, so it is a safe way to see whether the setup itself is the problem without any risk of it accidentally becoming the record.
